  ...
  ensures committed == old(committed) + PendingSum(old(reservedLog))
{
  ...
  //     invariant committed == startCommitted
  //     // 4) The original committed amount plus the full pending total fits within capacity, so the eventual final commit is safe.
  //     invariant committed + PendingSum(pending) <= total
  //     // 5) Total inventory is constant throughout this method (and specifically unchanged from method entry).
  //     invariant total == old(total)
  //     decreases |pending| - i
  //   {
...
// Prove iterating through a while loop and summing up the first k items of pending is the same as PendingSum(pending[..k]) 
// ensures sum == PendingSum(pending[..k])
// while i < k
//   invariant 0 <= i <= k
//   invariant sum == PendingSum(pending[..i])
//   decreases k - i
// {
//   sum := sum + pending[i];
...
// }
// assert i == k;
// assert sum == PendingSum(pending[..i]);
...
//       i := i + 1;
//     }
//     // var sum := PrefixCommit(pending, i);
//     // assert sum == PendingSum(pending[..i]);
//     // assert sum == newlyCommitted;
//     // assert newlyCommitted == PendingSum(pending[..i]);
...
//     taken := taken + pending[i];
//     i := i + 1;
//     PendingSumConcat(pending[..i-1], [pending[i-1]]);
//     assert pending[..k] + pending[k..] == pending;

//     assert taken == PendingSum(pending[..i]);
//   }
