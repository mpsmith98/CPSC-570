module HomeworkExtraction where
...
count_items :: (List Line_item) -> Nat
count_items xs =
  case xs of {
   Nil -> O;
   Cons _ rest -> add (S O) (count_items rest)}

order_item_count :: Order -> Nat
order_item_count o =
  count_items (items o)

quantity_is_positive :: Line_item -> Bool
quantity_is_positive item =
  case quantity item of {
   O -> False;
   S _ -> True}

all_quantities_positive :: (List Line_item) -> Bool
all_quantities_positive xs =
  case xs of {
   Nil -> True;
   Cons item rest ->
    andb (quantity_is_positive item) (all_quantities_positive rest)}

data Homework_summary =
   Build_homework_summary Nat String Nat Nat Bool Bool Bool

homework_summary_of_order :: Order -> Homework_summary
homework_summary_of_order o =
  Build_homework_summary (order_id o) (customer o) (order_item_count o)
    (total_quantity o) (validation_is_valid (validate_order o))
    (all_quantities_positive (items o)) (order_has_rush_fee o)
